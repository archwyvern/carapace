...
export const CANCEL_BTN = "px-4 py-1.5 text-xs text-fg-mid hover:text-fg";

export function confirmBtn(opts: { danger?: boolean; disabled?: boolean }): string {
  const base = "border px-5 py-1.5 text-xs uppercase tracking-wide";
...
export const DIALOG_INPUT =
  "mb-2 w-full rounded-control border border-border bg-surface-sunken px-2.5 py-1 text-xs text-fg shadow-[inset_0_1px_2px_rgba(0,0,0,0.45)] outline-none focus:border-accent";
