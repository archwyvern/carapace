    ...
    >
      ...
      >
        <input
          ...
          className="w-full rounded-control border border-border bg-surface-sunken px-3 py-2 text-base text-fg shadow-[inset_0_1px_2px_rgba(0,0,0,0.45)] outline-none focus:border-accent"
        />
        <ul role="listbox" className="max-h-80 overflow-auto py-1">
          {results.length === 0 && (
            <li className="px-3 py-2 text-base text-fg-mid">No matching commands</li>
          )}
          {results.map((cmd, i) => (
            <li key={cmd.id}>
              <button
                ...
                onClick={() => choose(i)}
                className={`flex w-full items-center gap-2 px-3 py-1 text-left text-base ${
