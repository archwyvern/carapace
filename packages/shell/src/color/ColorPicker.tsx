...
export function ColorPicker({ value, hasAlpha, onChange }: ColorPickerProps) {
  ...
  return (
    <div className="flex w-[280px] flex-col gap-2 border border-border bg-surface-raised p-2 text-base">
      ...
      <div className="flex items-center gap-1">
        <span className="w-7 text-base text-fg-mid">Hex</span>
        <span className="text-base text-fg-mid">#</span>
        <input
          type="text"
          value={currentHex.slice(1)}
          onChange={(e) => handleHexChange(e.target.value)}
          maxLength={6}
          className="h-[22px] flex-1 border border-border bg-surface px-1 font-mono text-base text-fg outline-none"
        />
      </div>
    </div>
  );
