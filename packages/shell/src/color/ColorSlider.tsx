...
export function ColorSlider({ label, value, min, max, step, color, gradient, onChange }: ColorSliderProps) {
  ...
  return (
    <div className="flex items-center gap-1.5" style={{ height: 24 }}>
      <span className="shrink-0 text-sm font-semibold" style={{ width: 12, color }}>
        {label}
      </span>
      <div
        ref={trackRef}
      ...
      <input
        ...
        className="h-[22px] shrink-0 border border-border bg-surface px-1 text-right text-sm text-fg outline-none"
        style={{ width: 52 }}
      />
    </div>
  );
